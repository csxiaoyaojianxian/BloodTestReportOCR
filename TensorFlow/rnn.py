...
with tf.Session() as sess:
    ...
    while step < training_iters:
        ...
        sess.run(optimizer, feed_dict={x: train_data, y: train_label_sex})
        if step % display_step == 0:
            # Calculate batch accuracy
            acc = sess.run(accuracy, feed_dict={x: train_data, y: train_label_sex})
            # Calculate batch loss
            loss = sess.run(cost, feed_dict={x: train_data, y: train_label_sex})
